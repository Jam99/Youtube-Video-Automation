...
def custom_upsize(clip, newsize=None, height=2112, width=1188, apply_to_mask=True):
    """ 
    ...
    if newsize is not None:
        ...
        if hasattr(newsize, "__call__"):
            ...
            return clip.fl(fun, keep_duration=True,
                           apply_to= (["mask"] if apply_to_mask else [])).set_position("center", "center")
        ...
        if hasattr(height, "__call__"):
            ...
            return custom_upsize(clip, fun).set_position("center", "center")
    ...
    if apply_to_mask and clip.mask is not None:
        newclip.mask = custom_upsize(clip.mask, newsize, apply_to_mask=False)

    return newclip.set_position("center", "center")


if not resize_possible:
    
    doc = custom_upsize.__doc__
    def custom_upsize(clip, newsize=None, height=None, width=None):
        raise ImportError("fx resize needs OpenCV or Scipy or PIL")
    custom_upsize.__doc__ = doc





def custom_downsize(clip, newsize=None, height=1920, width=1080, apply_to_mask=True):
    """ 
    Returns a video clip that is a resized version of the clip.
    ...
    if newsize is not None:
        ...
        if hasattr(height, "__call__"):
            fun = lambda t : 1.0*int(height(t))/h
            return custom_downsize(clip, fun).set_position("center", "center")
    ...
    if apply_to_mask and clip.mask is not None:
        newclip.mask = custom_downsize(clip.mask, newsize, apply_to_mask=False)
...
if not resize_possible:
    ...
    doc = custom_downsize.__doc__
    def custom_downsize(clip, newsize=None, height=None, width=None):
        raise ImportError("fx resize needs OpenCV or Scipy or PIL")
    custom_downsize.__doc__ = doc
